...
from analytics.tasks import generate_report_synchronously
from products.models import Product
from sales.models import Invoice, SalesOrder, SalesOrderItem
from trading.models import Order, Transaction
from trading.utils import attempt_to_match_order
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            order = SalesOrder.objects.create(
                customer=request.user, status="pending", discount_percent=0
            )
            # create the order item
            SalesOrderItem.objects.create(
                sales_order=order, product=product, quantity=quantity, price=price
            )
...
def update_product_view(request, product_id):
    ...
    return render(
        request, "frontend/update_product.html", {"form": form, "product": product}
    )
...
def create_trading_order_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"{order.order_type.capitalize()} order #{order.id} placed successfully.",
            )
...
def create_transaction_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Transaction #{transaction.id} created for Order #{transaction.order.id}.",
            )
...
def list_transactions_view(request):
    ...
    if request.user.role in ["admin", "trader"]:
        transactions = (
            Transaction.objects.select_related("order").all().order_by("-executed_at")
        )
    else:
    ...
    return render(
        request, "frontend/list_transactions.html", {"transactions": transactions}
    )
...
def generate_invoice_frontend_view(request, order_id):
    ...
    if request.user != sales_order.customer and user_role not in ["admin", "sales"]:
        return HttpResponseForbidden(
            "You do not have permission to generate an invoice for this order."
        )
